...
public class MyBot extends ChallengeBot {
  ...
  public void executeTurn(World world, Controller controller) {
    ...
    if (world.getHand().isEmpty() && world.getRedrawTime() > 5) {
      ...
      if (redrawable)
        if ((!money || !food || !materials) && perc_money > 0.45 &&
            perc_food > 0.43 && perc_materials > 0.45)
          controller.redraw();
    }
    ...
    for (var card : world.getHand()) {
      ...
      if (card == TileType.Grass) {
        ...
        // see docs
        int best_count_windmills_per_wheat =
            Integer.MAX_VALUE;                     // should be lowest
